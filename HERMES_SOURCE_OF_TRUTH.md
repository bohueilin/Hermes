...
| Authorize the MetaDrive longer-map derivation? | The P0 `fcw_stationary_lead` attempt hit a hard stop 2026-08-26: the adapter hardcodes `map: "S"` (`metadrive.py:331`, mirrored at `verification.py:1075`), whose seed-7 extent cannot spawn a stationary actor past a ~120 m gap (measured; 125 fails), and the FCW warning-margin design needs ~140 m (measured margin at the 120 m ceiling: 1.9 s < the 2.0 s floor). Proposed fix (probed end-to-end, measured margin 2.9 s on a longer map): a pure shared derivation in `simulator_support.py` keeping every committed scenario byte-identical on `"S"`; no adapter version bump per the `ad922fc` precedent. Reviewed prompt ready at `…/Hermes-adas/.superpowers/sdd/phase8-p0-fcw-scenarios/codex-implementation-prompt-v2.md` (ignored dir); handing it to Codex constitutes the approval. Also unblocks future long-approach `steady_lead` scenarios. |
