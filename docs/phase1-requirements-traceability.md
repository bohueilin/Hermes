...
## Phase 4 and Phase 5 extension

| ID | Hazard or requirement | Scenario/input | Component and verifier | Automated evidence/test | Gate consequence |
|---|---|---|---|---|---|
| P4-01 | Observation faults must preserve exact source and delivery provenance | `fake_fault_injection.yaml` | `DeterministicFaultInjector`; schema-2 typed observation evidence | Delay/freeze/dropout/noise unit tests; fault integration artifact assertions | Contradictory transform is `INVALID_EVIDENCE` / 30 |
| P4-02 | Frozen/dropped packets must not change sensor values under noise | Combined source delay, freeze, dropout, bounded noise | Source-packet keyed counter noise | `test_noise_is_bound_to_the_source_packet_so_delayed_frozen_and_dropped_values_hold` | Exact replay rejects a changed packet |
| P4-03 | Shield permission and actuator faults cannot be conflated | Control delay plus steering/brake saturation | Candidate → permitted → executed schema; fault metrics | Distinct-action artifact assertions and transform unit tests | Shield counts exclude control faults |
| P4-04 | Every configured mechanism and scheduled step must execute | Early destination/collision before remaining schedule | `FaultCoverageVerifier` | Early-terminal and partial-dropout-schedule integration tests | Required `NOT_AVAILABLE` forces `HOLD` / 20 |
| P4-05 | Full-hash rewrite must not forge deterministic fault transformations | Changed action/source/reason/permitted chain with refreshed hashes | Stored-only exact fault/shield replay | Four coherent-rehash parameter cases | `INVALID_EVIDENCE` / 30 |
| P4-06 | Adapter raw/result timing and freshness must remain truthful | Changed raw age or result sequence/time/age | Semantic trace verifier | Coherent-rehash raw/result tests | `INVALID_EVIDENCE` / 30 |
| P4-07 | Mixed schema evidence must fail closed, not crash | Schema-2 trace with a schema-1 event | Version-aware parser and recomputation guard | Mixed-schema coherent bundle test | `INVALID_EVIDENCE` / 30 |
| P4-08 | MetaDrive observation faults must affect the selected policy or be rejected | Schema-3 MetaDrive IDM observation-fault profile | Orchestrator profile validation | Pre-construction rejection test | Configuration error / 40; adapter not launched |
| P4-09 | Schema-3 challenge faults must retain actor truth checks | Action-only fault challenge with missing/false actor fields | Challenge pairing/phase/continuity verification | Valid and three adversarial schema-3 challenge tests | `INVALID_EVIDENCE` / 30 |
| P4-10 | Faulted artifacts must compare only under the same profile | Equal/different fault digests | Comparison compatibility | `test_fault_profile_identity_is_required_for_comparison` | Different profile refuses comparison / 40 |
| P5-01 | Persisted schema versions cannot default silently | Missing, unsupported, parent/nested mismatch in schema 1 and 2 | Explicit version dispatcher | `test_artifact_schema_version.py` | `INVALID_EVIDENCE` / 30 |
| P5-02 | CLI failures must be machine-stable and never print false PASS | Usage, configuration, operational, invalid, incompatible paths | `cli_errors.py`; Typer group | `test_cli_errors.py` plus existing CLI suite | Stable category with exit 30 or 40 |
| P5-03 | PR CI must not depend on real MetaDrive | GitHub Actions Python 3.11 job | `metadrive` pytest marker | CI command `pytest -m "not metadrive"`; strict markers | CI excludes real simulator launches |
| P5-04 | Local demo and simulator checks must be explicit and non-overwriting | Unique `DEMO_RUN_ID`; pinned local simulator | Make targets | Actual `make demo-phase1`; actual `make sim-smoke` | Existing artifact refuses overwrite; smoke is not a verdict |
