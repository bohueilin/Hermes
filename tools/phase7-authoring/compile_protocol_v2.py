...
BODY = {
    ...
    "registration": {
        "repository_relative_path": "evaluation-plans/lead_ttc_engagement.protocol.v2.yaml"
    },
}

# Version 2 grid. Motivated only by the committed v1 discovery ledger and by
# retained pre-existing artifacts:
#   - v1 showed 6-10 m gaps keep the ego gap-limited near 2.1-2.5 m/s, so the
#     closing speed never grows and the TTC ratio stays above 11 s;
#   - retained handoff-p2-metadrive shows the ego needs about 20.8 m of free
#     travel to reach its 8.0 m/s target.
# v2 therefore opens the gap so the ego can reach cruise, and triggers the brake
# well after cruise is established.
GRID = (
    ("initial_gap_m", (12.0, 16.0, 20.0, 30.0)),
    ("actor_speed_mps", (6.0, 8.0)),
    ("trigger_step", (80, 140)),
    ("brake_duration_steps", (30, 60)),
...
(out / "lead_ttc_engagement.protocol.v2.yaml").write_bytes(result.protocol_yaml_bytes)
