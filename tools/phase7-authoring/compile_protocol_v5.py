...
SHIELD = ROOT / "config" / "shield.phase7.lead_ttc.v2.yaml"
...
# The metadrive-idm policy has a real configuration. Its digest has been stable
# since Phase 2 and is read from retained pre-existing artifacts, never from a
# Phase 7 result. v4 wrongly declared the empty-object digest and the assessor
# correctly refused the pair.
POLICY_CONFIG_DIGEST = "22b5e129ed53fad94e0bf70e38bdf341316d8ff1d8a75652abd08c964f230fa4"
retained = json.loads(
    (ROOT / "artifacts" / "handoff-p2-metadrive" / "manifest.json").read_text()
)
if retained["policy_config_digest"] != POLICY_CONFIG_DIGEST:
    raise SystemExit("retained policy digest does not match the declared constant")
...
BODY = {
    ...
    "registration": {
        "repository_relative_path": "evaluation-plans/lead_ttc_engagement.protocol.v5.yaml"
    },
}

# Version 5 grid. Identical question and grid to v4; the only change is the
# corrected policy component identity that v4 mis-declared.
# Version 4 grid. Concentrated on the floor the v2 and v3 ledgers bracketed:
# the observed minimum policy-input TTC is flat near 3.11 s at 30-40 m and rises
# on both sides. The declared threshold is 4.0 s, above that measured floor, so
# the question is reachable and the assessor's positive path can be exercised.
# The 2.0 s question remains closed as unreachable in DISCOVERY_RESULTS.md.
GRID = (
    ("initial_gap_m", (30.0, 40.0, 50.0)),
    ("actor_speed_mps", (8.0,)),
...
(out / "lead_ttc_engagement.protocol.v5.yaml").write_bytes(result.protocol_yaml_bytes)
