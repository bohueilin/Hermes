...
protocol = StudyProtocol.model_validate_json(
    json.dumps(
        yaml.safe_load(
            (ROOT / "evaluation-plans" / "lead_ttc_engagement.protocol.v4.yaml").read_text()
        ),
        separators=(",", ":"),
        sort_keys=True,
    )
...
pair_plan = PairPlan.model_validate_json(
    json.dumps(
        yaml.safe_load(
            (ROOT / "evaluation-plans" / "lead_ttc_engagement.pair.v4.yaml").read_text()
        ),
        separators=(",", ":"),
        sort_keys=True,
    )
...
ledger = [
    json.loads(line)
    for line in (ROOT / "evaluation-plans" / "lead_ttc_engagement.discovery.v4.jsonl")
    .read_text()
    .splitlines()
    if line.strip()
]
